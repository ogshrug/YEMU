...
class VMPrepareWindow(Gtk.Window):
    def __init__(self, parent=None):
        super().__init__(title="VM Preparation Tool", transient_for=parent, modal=True)
        ...
        # VM Name
        self.name_entry = Adw.EntryRow(title="VM Name", text="ubuntu-clean")
        group.add(self.name_entry)

        # Distro Selection
        self.distro_combo = Gtk.DropDown.new_from_strings(["ubuntu", "debian", "windows"])
    ...
    def _on_start_clicked(self, btn):
        ...
        distro = self.distro_combo.get_selected_item().get_string()
        ram = int(self.ram_spin.get_value())
        cpu = int(self.cpu_spin.get_value())
        disk_size = int(self.disk_spin.get_value())

        threading.Thread(target=self._run_preparation, args=(vm_name, distro, ram, cpu, disk_size), daemon=True).start()
    ...
    async def _async_prepare(self, vm_name, distro, ram, cpu, disk_size):
        ...
        try:
            ...
            if distro in ["ubuntu", "debian"]:
                ...
                # Extract procmon for injection later
            ...
            backing = image_path if distro in ["ubuntu", "debian"] else None
            ...
            if not await self.manager.define_vm(xml):
            ...
            GLib.idle_add(self._append_log, "Waiting for Guest Agent to become ready (this may take 5-10 minutes)...")
            GLib.idle_add(self.progress_bar.set_fraction, 0.7)

            if not await self.manager.wait_for_guest_agent(vm_name, timeout=900): # 15 min timeout for install
                 raise RuntimeError("Guest agent timeout. Installation may have failed.")

            # 6. Post-Installation Tools
            if distro == "windows" and procmon_path:
                GLib.idle_add(self._append_log, "Installing Process Monitor and QEMU Guest Agent in Windows guest...")
                # For Windows, we need to install the guest agent first if it's not there,
                # but our Autounattend should have handled basic setup if we had a way to run scripts.
                # Since we are using virt-copy-in, we stop the VM.
                ...
                # Also try to inject and install guest agent from virtio-win if possible,
                # but for now we assume the user might need to finish manual steps if GA doesn't start.
                # However, let's try to make it work.
