...
class ScoreGauge(QWidget):
    ...
    def paintEvent(self, _):
        ...
        f.setBold(True)
        p.setFont(f)
        p.drawText(self.rect().adjusted(0, -int(self._size * 0.06), 0, 0), Qt.AlignCenter, str(self._score))
        ...
        muted.setAlpha(140)
        p.setPen(muted)
        p.drawText(self.rect().adjusted(0, int(self._size * 0.32), 0, 0), Qt.AlignCenter, "/ 100")
...
class DropZone(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.hint = label("or click to browse. Any file type; it is copied into the VM, never run on this host.", "Muted", wrap=True)
...
class StageTracker(QWidget):
    ...
    COLORS = {"pending": None, "active": "#7b73ff", "done": "#16a34a", "warn": "#d97706", "failed": "#dc2626", "skipped": None}
    ...
    def set_state(self, key, state):
        ...
        text.setStyleSheet("font-weight: 600;" if state == "active" else ("color: palette(mid);" if state in ("pending", "skipped") else ""))
