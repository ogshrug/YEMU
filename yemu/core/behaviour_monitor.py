...
class BehaviourMonitor:
    ...
    def parse_strace(self, log_lines, pid=None):
        ...
        for line in log_lines:
            ...
            proc_info = self.process_info.get(
                current_pid,
                {"process_name": "unknown", "exe_path": "[unreadable]", "cmdline": "[unreadable]", "ppid": "unknown"},
            )
            ...
            if 'execve' in line:
                ...
                if match:
                    ...
                    events.append(
                        {**base_event, "type": "process", "action": "execute", "path": exe_path, "syscall": "execve"}
                    )
                ...
                if match:
                    events.append(
                        {**base_event, "type": "file", "action": "delete", "path": match.group(1), "syscall": "unlink"}
                    )
            elif 'write' in line:
                ...
                if match:
                    events.append(
                        {**base_event, "type": "file", "action": "write", "fd": match.group(1), "syscall": "write"}
                    )
        return events
